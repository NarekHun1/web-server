    ...
    fastify.post('/users/init', async (_, reply) => {
        ...
        try {
            ...
            if (res.rows.length > 0) {
                await client.query('UPDATE users SET balance = $1 WHERE id = $2', [initialBalance, userId]);
                return reply.send({ message: 'User balance reset to 100', id: userId, balance: initialBalance });
            } else {
                await client.query('INSERT INTO users (id, balance) VALUES ($1, $2)', [userId, initialBalance]);
